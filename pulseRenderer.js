...
function translateStaticPage() {
    document.title = t('listen.window.title', 'Aurivo Dinle');
...
function getListenModePreset(mode) {
    ...
    if (key === 'normal') return { request_interval_secs_v3: 5, buffer_size_secs: 12 };
    if (key === 'max') return { request_interval_secs_v3: 3, buffer_size_secs: 16 };
    return { request_interval_secs_v3: 4, buffer_size_secs: 14 };
}
...
function syncPreferenceForm() {
    ...
    if (els.requestInterval) els.requestInterval.value = String(preferences.request_interval_secs_v3 || 4);
    if (els.bufferSize) els.bufferSize.value = String(preferences.buffer_size_secs || 14);
...
async function loadPreferences() {
    const res = await window.aurivo?.pulse?.getPreferences?.().catch(() => null);
...
async function savePreferencePatch(patch) {
    ...
    next.request_interval_secs_v3 = Math.max(1, Math.min(120, Number(next.request_interval_secs_v3) || 4));
    next.buffer_size_secs = Math.max(4, Math.min(30, Number(next.buffer_size_secs) || 14));
    next.recognition_engine = 'songrec_only';
    const res = await window.aurivo?.pulse?.savePreferences?.(next);
...
async function refreshDevices() {
    ...
    const res = await window.aurivo?.pulse?.listDevices?.();
    renderDevices(res?.devices || []);
    const pref = await window.aurivo?.pulse?.getPreferredDevice?.().catch(() => null);
    ...
    } else if (pref?.audioDevice && lastDevices.some((device) => device.id === pref.audioDevice)) {
...
function addResult(result) {
    ...
    button.addEventListener('click', () => {
        ...
        window.aurivo?.pulse?.openQueryInApp?.({ query, platform: selectedOpenPlatform(), result });
    });
...
window.aurivo?.pulse?.onState?.((state) => {
...
window.aurivo?.pulse?.onResult?.(addResult);
window.aurivo?.pulse?.onResult?.(() => {
    setListenCopyKey(
        'listen.state.found.title',
        'Bulundu',
        'listen.state.found.subtitle',
        'Sonuç aşağıdaki listeye eklendi dinlemeye devam ediyorum'
    );
});
window.aurivo?.pulse?.onVolume?.((payload) => {
...
window.aurivo?.pulse?.onUncertain?.((payload) => {
