...
const backgroundStyles={builtin:"url('ardali-flow-blue.png')",'solid-indigo':'linear-gradient(#2638b8,#2638b8)','solid-magenta':'linear-gradient(#a51d83,#a51d83)','solid-teal':'linear-gradient(#087f78,#087f78)','solid-ember':'linear-gradient(#a34420,#a34420)','gradient-violet':'linear-gradient(135deg,#251558,#7d2fe4 55%,#da3198)','gradient-aurora':'linear-gradient(135deg,#063d62,#087f78 55%,#69c67a)','gradient-ocean':'linear-gradient(135deg,#172a77,#176ca4 52%,#26b3c7)','gradient-sunset':'linear-gradient(135deg,#5b1838,#c44b43 52%,#f0a13b)'};
...
let p=Object.assign({},defaults,stored);if(stored.backgroundPreferenceSet!==true)p.backgroundVisible=false;if(!Array.isArray(p.hiddenFrequentSites))p.hiddenFrequentSites=[];if(!['session','all_time'].includes(p.blockedCounterMode))p.blockedCounterMode='all_time';const clamp=value=>Math.max(0,Math.min(100,Number(value)||0));const save=()=>localStorage.setItem('ardali.newtab',JSON.stringify(p));
...
window.ardaliCardData=window.ardaliCardData||{downloads:__ARDALI_DOWNLOAD_COUNT__,blockedAllTime:__ARDALI_BLOCKED_COUNT__,blockedSession:__ARDALI_BLOCKED_SESSION_COUNT__,blocked:__ARDALI_BLOCKED_COUNT__};
function renderCardValues(){const isSession=(p.blockedCounterMode==='session');const blockedVal=isSession?(window.ardaliCardData.blockedSession||0):(window.ardaliCardData.blockedAllTime||0);const scopeText=isSession?'Oturum boyunca':'Tüm zamanlar';const scopeEl=$('#protection-card-scope');if(scopeEl)scopeEl.textContent=scopeText;const valEl=$('#protection-card-value');if(valEl)valEl.textContent=Math.max(0,Number(blockedVal)||0).toLocaleString();const dlEl=$('#downloads-card-value');if(dlEl)dlEl.textContent=String(window.ardaliCardData.downloads||0)}
window.ardaliSetProtectionStats=(totalBlocked,downloads,sessionBlocked)=>{window.ardaliCardData.blockedAllTime=Math.max(0,Number(totalBlocked)||0);window.ardaliCardData.downloads=Math.max(0,Number(downloads)||0);if(sessionBlocked!==undefined){window.ardaliCardData.blockedSession=Math.max(0,Number(sessionBlocked)||0)}window.ardaliCardData.blocked=window.ardaliCardData.blockedAllTime;renderCardValues()};
window.ardaliSetCardSettings=(showDownloads,showBlocked,mode)=>{if(typeof showDownloads==='boolean')p.showDownloadsCard=showDownloads;if(typeof showBlocked==='boolean')p.showBlockedCard=showBlocked;if(mode==='session'||mode==='all_time')p.blockedCounterMode=mode;p.cards=(p.showDownloadsCard||p.showBlockedCard);save();render()};
...
function backgroundCommand(op){if(!suggestionCapability)return false;location.href='ardali://newtab-background?op='+encodeURIComponent(op)+'&cap='+encodeURIComponent(suggestionCapability);return true}
...
window.ardaliSetManagedBackgroundState=(available,revision)=>{window.ardaliManagedBackgroundAvailable=!!available;window.ardaliManagedBackgroundRevision=String(revision||0);if(!available&&p.backgroundSource==='custom')p.backgroundSource='builtin';save();render()};
window.ardaliBackgroundResult=(ok,message,available,revision,selectCustom)=>{window.ardaliManagedBackgroundAvailable=!!available;window.ardaliManagedBackgroundRevision=String(revision||0);const status=$('#background-status');status.textContent=message||'';status.classList.toggle('error',!ok);if(ok&&available&&selectCustom){p.backgroundSource='custom';p.backgroundVisible=true;p.backgroundPreferenceSet=true;p.theme='flow'}if(!available&&p.backgroundSource==='custom')p.backgroundSource='builtin';settingsChanged();render()};
window.ardaliBackgroundCancelled=()=>{$('#background-status').textContent=''};
$('#suggestions-toggle').onchange=event=>{window.ardaliSuggestionsEnabled=event.target.checked;clearTimeout(suggestionRequestTimer);++suggestionId;closeSuggestions();if(!event.target.checked){suggestionRows=[];suggestionList.replaceChildren()}suggestionCommand('consent',{enabled:String(event.target.checked)});};
...
function suggestionCommandUrl(op,params={}){if(!suggestionCapability)return'';return'ardali://suggest?'+new URLSearchParams({op,cap:suggestionCapability,...params}).toString().replace(/\+/g,'%20')}
function suggestionCommand(op,params={}){const url=suggestionCommandUrl(op,params);if(url)location.href=url}
...
function requestSuggestions(){clearTimeout(suggestionRequestTimer);suggestionCommitPending=false;suggestionPreviewActive=false;suggestionTypedValue=query.value;++suggestionId;if(window.ardaliSuggestionsEnabled!==true){suggestionRows=[];suggestionList.replaceChildren();closeSuggestions();return}if(document.activeElement!==query)return;suggestionCommand('query',{q:suggestionTypedValue.slice(0,256),id:String(suggestionId)})}
function scheduleSuggestions(){clearTimeout(suggestionRequestTimer);suggestionCommitPending=false;suggestionPreviewActive=false;suggestionTypedValue=query.value;if(window.ardaliSuggestionsEnabled!==true){suggestionRows=[];suggestionList.replaceChildren();closeSuggestions();return}suggestionRequestTimer=setTimeout(requestSuggestions,80)}
...
window.ardaliSuggestionConsent=(enabled,allowed=true)=>{window.ardaliSuggestionsEnabled=enabled===true;$('#suggestions-toggle').disabled=!allowed;render();clearTimeout(suggestionRequestTimer);++suggestionId;closeSuggestions();if(!window.ardaliSuggestionsEnabled){suggestionRows=[];suggestionList.replaceChildren();return}if(document.activeElement===query)requestSuggestions()};
...
$('#search').onsubmit=event=>{event.preventDefault();const value=$('#query').value.trim();if(!value)return;const engine=$('#engine').value||'DuckDuckGo';location.href='ardali://navigate?q='+encodeURIComponent(value)+'&engine='+encodeURIComponent(engine)+'&cap='+encodeURIComponent(suggestionCapability);};
...
setEngine($('#engine').value);applyFrequentConfig();tick();setInterval(tick,1000);
  ...
  const bool customDefault = defaultEngine != QLatin1String("Google")
      && defaultEngine != QLatin1String("DuckDuckGo")
      && defaultEngine != QLatin1String("Brave Search")
      && defaultEngine != QLatin1String("Bing");
  if (customDefault) {
    const QString escaped = defaultEngine.toHtmlEscaped();
  ...
  if (customDefault) {
    placeholders.append(jsonStringLiteral(defaultEngine) + QLatin1Char(':')
                        + jsonStringLiteral(QStringLiteral("%1 ile arayın veya URL'yi yazın").arg(defaultEngine)));
    engineIcons.append(jsonStringLiteral(defaultEngine) + QLatin1Char(':')
  ...
  html.replace(QLatin1String("__ARDALI_BLOCKED_COUNT__"), QString::number(totalBlockedCount));
  html.replace(QLatin1String("__ARDALI_BLOCKED_SESSION_COUNT__"), QString::number(sessionBlockedCount));
  html.replace(QLatin1String("__ARDALI_DOWNLOAD_COUNT__"),
...
QString newTabProtectionStatsUpdateScript(quint64 totalBlockedCount,
  ...
  if (sessionBlockedCount >= 0) {
    return QStringLiteral("if(window.ardaliSetProtectionStats)window.ardaliSetProtectionStats(%1,%2,%3);")
        .arg(totalBlockedCount)
        .arg(std::max(0, recentDownloadCount))
        .arg(sessionBlockedCount);
  }
  return QStringLiteral("if(window.ardaliSetProtectionStats)window.ardaliSetProtectionStats(%1,%2);")
      .arg(totalBlockedCount)
      .arg(std::max(0, recentDownloadCount));
}
...
QString newTabCardSettingsUpdateScript(bool showDownloadsCard,
  ...
  return QStringLiteral("if(window.ardaliSetCardSettings)window.ardaliSetCardSettings(%1,%2,%3);")
...
QString newTabBackgroundStateUpdateScript(bool available, quint64 revision) {
  return QStringLiteral("if(window.ardaliSetManagedBackgroundState)window.ardaliSetManagedBackgroundState(%1,%2);")
...
QString newTabBackgroundResultScript(bool ok, const QString &message,
                                     ...
                                     bool selectCustom) {
  return QStringLiteral("if(window.ardaliBackgroundResult)window.ardaliBackgroundResult(%1,%2,%3,%4,%5);")
