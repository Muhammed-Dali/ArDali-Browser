...
QWidget *SettingsPage::createPasswordsSection() {
  Section section = makeSection(QStringLiteral("Şifreler ve otomatik doldurma"), QStringLiteral("Kimlik bilgileri ve form verileri için ayrılmış alan."));
  section.layout->addWidget(placeholderPanel(section.page, BrowserIcon::Password, QStringLiteral("Password Manager entegrasyonu daha sonra eklenecek"), QStringLiteral("ArDali Password Manager henüz bu tarayıcıya bağlanmadı. Bu sayfa kayıt üretmez ve şifre saklamaz.")));
  section.layout->addStretch(); return section.page;
}
