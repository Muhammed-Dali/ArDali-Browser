...
int main(int argc, char *argv[]) {
  ...
  if (restored.size() != 1 || restored.front().url != QUrl(QStringLiteral("https://example.com/session"))) return 1;
  // The disposable Web Audio runtime fixture must not be restored on normal startup.
  manager.updateUrl(secondId, QUrl(QStringLiteral("http://127.0.0.1:18766/")));
