...
int main() {
  QString error;
  const BrowserPolicy policy = BrowserPolicy::load(QStringLiteral(DALINIRA_BROWSER_POLICY_PATH), &error);
  if (!policy.isValid() || !policy.allowsNavigation(QUrl("https://example.com"))
      || !policy.allowsNavigation(QUrl("dalinira://newtab"))
      || policy.allowsNavigation(QUrl("dalinira://attacker-controlled"))
