...
inline QString translateEngineScript() {
  // First attempt to load from single-source Qt resource if available
  QFile resFile(QStringLiteral(":/translate/native/translate/resources/translate_engine.js"));
  if (resFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
    return QString::fromUtf8(resFile.readAll());
  }

  // Also check local path
  QFile localFile(QStringLiteral("browser/native/translate/resources/translate_engine.js"));
