...
BrowserProfileService::BrowserProfileService(const QString &dataDirectory, const BrowserPolicy *policy, QObject *parent)
  ...
  interceptor_ = new TrackingParameterInterceptor(this);
  static_cast<TrackingParameterInterceptor *>(interceptor_)->setEnabled(stripsTrackingParameters());
...
void BrowserProfileService::handleDownload(QWebEngineDownloadRequest *download) {
  ...
  connect(download, &QWebEngineDownloadRequest::stateChanged, this, [this, download, path = info.absoluteFilePath()](QWebEngineDownloadRequest::DownloadState state) {
...
void BrowserProfileService::setStripsTrackingParameters(bool enabled) {
  ...
  preferences_.sync();
  static_cast<TrackingParameterInterceptor *>(interceptor_)->setEnabled(enabled);
...
void BrowserProfileService::recordHistory(const QUrl &url, const QString &title) {
  ...
  for (const QJsonObject &site : orderedSites.first(std::min<qsizetype>(100, orderedSites.size()))) frequentValues.append(site);
...
QList<BrowserFrequentSite> BrowserProfileService::frequentSites(int limit) const {
  ...
  return sites.first(std::min<qsizetype>(limit, sites.size()));
}
