# DaliNira Browser 7.1.1

Released on 13 September 2026.

DaliNira Browser 7.1.1 is a Linux packaging reliability update that restores all
New Tab artwork and controls in system-installed builds.

## New Tab asset fixes

- Corrected the installed-package path from the application under
  `/usr/lib/dalinira-browser` to its assets under `/usr/share/dalinira-browser`.
- Restored the DaliNira logo, search box artwork, search-engine icons,
...
`dalinira-browser-7.1.1-linux-x86_64.tar.zst`.
