...
Aurivo-Pulse is an open-source Shazam client for Linux, written in Rust.
...
The above decribes the newer CLI interface of Aurivo-Pulse, but an older interface, operating only on audio files or raw audio fingerprints, is also available and described below.
...
Aurivo-Pulse collects no data and contacts no other servers than Shazam's. Aurivo-Pulse does not upload raw audio data anywhere: only fingerprints of the audio are uploaded, which means sequences of frequency peaks encoded in the form of "(frequency, amplitude, time)" tuples.
