...
wine /opt/gtkwin/resource_hacker/ResourceHacker.exe -open ~/rust-shazam/packaging/windows/7zxSD_LZMA2_x64.sfx -save /tmp/Aurivo-Pulse-standalone.exe -action delete -mask ,101, -log CONSOLE
wine /opt/gtkwin/resource_hacker/ResourceHacker.exe -open /tmp/Aurivo-Pulse-standalone.exe -save /tmp/Aurivo-Pulse-standalone.exe -action addoverwrite -res ~/rust-shazam/packaging/windows/songrec.ico -mask ICONGROUP,MAINICON,0 -log CONSOLE
cat << EOF >> /tmp/Aurivo-Pulse-standalone.exe
;!@Install@!UTF-8!
ExecuteFile="songrec.exe"
GUIMode="2"
;!@InstallEnd@!
EOF
cat /tmp/songrec-files.7z >> /tmp/Aurivo-Pulse-standalone.exe

cp /tmp/Aurivo-Pulse-standalone.exe ~/win32/ # Copy the generated executable to my VirtualBox shared folder

# wine /tmp/Aurivo-Pulse-standalone.exe
