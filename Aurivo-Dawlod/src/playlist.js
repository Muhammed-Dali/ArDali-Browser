...
const playlistDownloader = {
	...
	loadInitialConfig() {
		...
		this.state.ffmpegPath = resolveFfmpegPathSync();
	...
	finishDownload(count) {
		...
		const notify = new Notification("Aurivo-Dawlod", {
	...
	getJsRuntimePath() {
		{
			...
			const nodeEnvPath =
				process.env.AURIVO_NODE_PATH ||
				process.env.YTDOWNLOADER_NODE_PATH;
			if (nodeEnvPath) {
			...
			const denoEnvPath =
				process.env.AURIVO_DENO_PATH ||
				process.env.YTDOWNLOADER_DENO_PATH;
			if (denoEnvPath) {
				if (fs.existsSync(denoEnvPath)) {
